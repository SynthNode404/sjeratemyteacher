...
interface EditTeacherModalProps {
  teacher: Teacher;
  onClose: () => void;
  onUpdated: (updatedTeacher: Teacher) => void;
...
export default function EditTeacherModal({
  ...
  const [accentColor, setAccentColor] = useState(teacher.accentColor || "emerald");
  const [uploadingImage, setUploadingImage] = useState<string | null>(teacher.avatar || null);
  ...
  useEffect(() => {
    ...
    setBio(teacher.bio || "");
    setAccentColor(teacher.accentColor || "emerald");
    setUploadingImage(teacher.avatar || null);
  }, [teacher]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAdminAuthenticated) {
      setErrorMessage("Only school administrators are authorized to update teacher profiles.");
      return;
    }
    ...
    try {
      const response = await fetch(`/api/admin/teachers/${teacher.id}/update`, {
        method: "POST",
        headers: {
  ...
  return (
    <div
      id="edit-teacher-modal-backdrop"
    ...
    >
      <div
        id="edit-teacher-modal-main"
        className="relative w-full max-w-lg overflow-hidden rounded-2xl bg-white dark:bg-slate-900 border border-transparent dark:border-slate-850 shadow-2xl animate-scale-in"
      >
        {/* Decorative Indicator Bar */}
        <div className={`h-2.5 w-full bg-gradient-to-r from-amber-500 to-indigo-600`} />

        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-805 p-6 dark:border-slate-800">
          <div>
            <h2 className="text-xl font-extrabold text-gray-900 dark:text-white">Edit Teacher Profile</h2>
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Ensure school directory details are current</p>
          </div>
          <button
            id="close-edit-teacher-modal"
            onClick={onClose}
        ...
        {!isAdminAuthenticated ? (
          <div className="p-8 text-center max-w-sm mx-auto">
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
              Editing teacher profiles is restricted to authorized school administrators. Verify using the administration password.
            </p>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="He teaches with passion."
              value={bio}
          ...
          <div className="flex gap-3 border-t border-gray-100 dark:border-slate-805 pt-5 dark:border-slate-805 border-slate-100">
            <button
              id="cancel-edit-teacher-btn"
              type="button"
              onClick={onClose}
              className="flex-grow rounded-xl border border-gray-200 py-2.5 text-sm font-semibold text-gray-750 dark:text-slate-350 hover:bg-gray-50 hover:text-gray-900 border-gray-200 dark:border-slate-800 dark:hover:bg-slate-800 dark:hover:text-white"
            >
              Cancel
            </button>
            <button
              id="confirm-edit-teacher-btn"
              type="submit"
              disabled={submitting}
